...
def random_weights(sizes):
    ...
    """
    return [
        xavier_initialization(sizes[i], sizes[i + 1]) for i in range(len(sizes) - 1)
    ]
...
def zeros_weights(sizes):
    ...
    """
    return [np.zeros(shape=(sizes[i], sizes[i + 1])) for i in range(len(sizes) - 1)]
...
def create_batches(data, labels, batch_size):
    ...
    return [
        (
            ...
            labels[i * batch_size : min((i + 1) * batch_size, len(data))],
        )
